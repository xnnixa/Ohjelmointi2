...
class H3Matcher<T> extends BaseMatcher<T> {
	 ...
	 */
	public H3Matcher(T expected, FailureMessage message) {
	...
	private static <T> H3Matcher<T> not(T expected, FailureMessage message) {
		H3Matcher<T> matcher = new H3Matcher<>(expected, message,
	...
	public static <T> void assertEquals(T actual, T expected,
		...
		makeAssertion(actual, new H3Matcher<>(expected, message));
	}
	...
	public static void assertReturns(Object actual, Object expected,
		...
		FailureMessage def = explain().expected(
									  ...
									  .but(methodName + " returned ⟨" + actual
											  + "⟩");
		makeAssertion(actual, new H3Matcher<>(expected, def.copyFrom(message)));
	}
	...
	public static <T extends Number> void assertWithin(T actual,
		...
		makeAssertion(expected.includes(actual),
				new H3Matcher<>(true, def.copyFrom(message))
		);
	}
	...
	public static <T extends Number> void assertWithout(T actual,
		...
		FailureMessage def = explain().expected(
									  ...
									  .but("was ⟨" + actual + "⟩");
		makeAssertion(expected.includes(actual),
				new H3Matcher<>(false, def.copyFrom(message))
		);
	}
	...
	private static <T> void makeAssertion(T actual, H3Matcher<T> matcher) {
