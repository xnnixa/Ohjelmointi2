...
class RegexMatcher extends BaseMatcher<String> {
	...
	private static final Map<String, String> metacharExplanations
			= Map.ofEntries(
			// @formatter:off
			entry("^", "Match start of line, negate inside character class"),
			entry("$", "Match end of line"),
			entry("(", "Start group (a \"capturing group\")"),
			entry(")", "End capturing group"),
			entry(".", "Match any character, literal '.' inside character class"),
			entry("[", "Start character class (e.g. [abc])"),
			entry("]", "End character class (e.g. [^A-Z])"),
			entry("-", "Character range (e.g. [a-z])"),
			entry("|", "Either or (e.g. cat|dog)"),
			entry("?", "Element to the left is optional (e.g. colou?r)"),
			entry("*", "Element to the left can occur zero or more times"),
			entry("+", "Element to the left can occur one or more times")
			// @formatter:on
	);
	...
	public static void assertMatches(Object actual, Pattern expected) {
		assertThat(Objects.toString(actual),
				new RegexMatcher(actual, expected)
		);
	}
	...
	static String regexExplanation(Pattern regex) {
		...
		for (Map.Entry<String, String> entry : metacharExplanations.entrySet()) {
